...
import { ArrowRight, Check, ChevronLeft, ExternalLink, Send, X } from "lucide-react";
...
function Mascot({ mood = "default", className = "" }: { mood?: string; className?: string }) {
  const moodAssets: Record<string, string> = { default: "1", wave: "1", "point-left": "2", "point-right": "3", thinking: "4", pacientes: "5", consultas: "5", chatbot: "chatbot" };
  const fallback = "/navi-svgs/1.svg";
  const [asset, setAsset] = useState(`/navi-svgs/${moodAssets[mood] ?? "1"}.svg`);
  useEffect(() => { setAsset(`/navi-svgs/${moodAssets[mood] ?? "1"}.svg`); }, [mood]);
...
export function ProductTour({ slug, userId, accountCreatedAt }: { slug?: string; userId: string; accountCreatedAt: string | null }) {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([{ role: "assistant", content: "Oi! Precisa de ajuda com a Reserve Clinic?" }]);
  ...
  async function sendMessage(content = chatInput) { const text = content.trim(); if (!text || chatSending) return; const nextMessages = [...chatMessages, { role: "user" as const, content: text }]; setChatMessages(nextMessages); setChatInput(""); setChatSending(true); try { const { data } = await supabase.auth.getSession(); if (!data.session) throw new Error("Sessão expirada. Entre novamente."); const response = await fetch("/api/navi-chat", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${data.session.access_token}` }, body: JSON.stringify({ messages: nextMessages }) }); const body = await response.json(); if (!response.ok) throw new Error(body.error ?? "Não foi possível responder agora."); setChatMessages((currentMessages) => [...currentMessages, { role: "assistant", content: body.message }]); } catch (error) { setChatMessages((currentMessages) => [...currentMessages, { role: "assistant", content: error instanceof Error ? error.message : "Não foi possível responder agora." }]); } finally { setChatSending(false); } }
  ...
  return <>{mode === "tour" && <div className="tourLayer" aria-live="polite"><div className="tourSpotlight" style={rect ? { top: rect.top - 8, left: rect.left - 8, width: rect.width + 16, height: rect.height + 16 } : undefined} /><section className="tourCard" style={cardStyle} role="dialog" aria-modal="true" aria-labelledby="tour-title"><div className="tourCardTop"><Mascot mood={current.mood} /><button className="tourCloseButton" onClick={dismiss} aria-label="Pular tutorial"><X size={16} /></button></div><span className="tourProgress">{step + 1} de {steps.length}</span><h2 id="tour-title">{current.title}</h2><p>{current.text}</p>{current.action && step === 2 && <button className="tourActionButton" onClick={() => router.push("/profissionais")}>{current.action} <ArrowRight size={15} /></button>}{current.action && step === 6 && slug && <a className="tourActionButton" href={`/agendamento/${slug}`} target="_blank" rel="noreferrer">{current.action} <ExternalLink size={14} /></a>}<div className="tourCardFooter"><button className="tourTextButton" onClick={previous} disabled={step === 0 && !agendaStage}><ChevronLeft size={15} /> Voltar</button><button className="tourPrimaryButton" onClick={next}>{step === steps.length - 1 ? "Concluir" : "Próximo"} <ArrowRight size={15} /></button></div><button className="tourSkipButton" onClick={dismiss}>Pular tutorial</button></section></div>}{(mode === "done" || mode === "dismissed") && <><button className="naviAssistantButton" onClick={() => setChatOpen(!chatOpen)} aria-label="Precisa de ajuda?" title="Precisa de ajuda?"><Mascot mood="chatbot" /></button>{chatOpen && <aside className="naviChatPanel" aria-label="Assistente Navi"><button className="tourCloseButton" onClick={() => setChatOpen(false)} aria-label="Fechar ajuda"><X size={16} /></button><Mascot mood="chatbot" /><strong>Oi! Precisa de ajuda?</strong><p>Posso te orientar na Reserve Clinic.</p><div className="naviMessages">{chatMessages.map((message, index) => <p key={`${message.role}-${index}`} className={`naviMessage naviMessage-${message.role}`}>{message.content}</p>)}{chatSending && <p className="naviMessage naviMessage-assistant">Navi está digitando...</p>}</div><div className="naviSuggestions"><button onClick={() => void sendMessage("Como criar uma consulta?")}>Como criar uma consulta?</button><button onClick={() => void sendMessage("Como cadastrar um profissional?")}>Como cadastrar um profissional?</button><button onClick={() => void sendMessage("Como configurar meus horários?")}>Como configurar meus horários?</button><button onClick={() => void sendMessage("Como compartilhar meu link?")}>Como compartilhar meu link?</button></div><form className="naviChatForm" onSubmit={(event) => { event.preventDefault(); void sendMessage(); }}><input value={chatInput} onChange={(event) => setChatInput(event.target.value)} placeholder="Digite sua dúvida" aria-label="Mensagem para o Navi" /><button type="submit" disabled={chatSending || !chatInput.trim()} aria-label="Enviar mensagem"><Send size={15} /></button></form></aside>}</>}</>;
}

export function SetupChecklist({ accountCreatedAt }: { accountCreatedAt: string | null }) {
  const [visible, setVisible] = useState(true);
  const [items] = useState([true, false, false, false, false]);
  const isNewAccount = Boolean(accountCreatedAt && Date.now() - new Date(accountCreatedAt).getTime() < 30 * 24 * 60 * 60 * 1000);
  if (!isNewAccount || !visible || items.every(Boolean)) return null;
