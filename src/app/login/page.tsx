  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error || !data.user) {
        ...
        if (status >= 500) {
          void reportarErroCliente("login", `[${status}] ${error?.message ?? "erro sem mensagem"}`);
        }
        setLoading(false);
      ...
      void reportarErroCliente("login", `throw: ${msg}`);
      setLoading(false);
      setError("Não foi possível entrar agora. Tenta novamente em instantes.");
    }
  }
